...
class Route {
	...
	public static function resource($route, $controller) {
		...
		self::get('/' . $route . '/new', '\\'.$controller . '@add');
		self::get('/' . $route . '/:(.*)/edit', '\\'.$controller . '@edit');
		self::get('/' . $route . '/:(.*)', '\\'.$controller . '@show');
		self::post('/' . $route, '\\'.$controller . '@create');
		self::put('/' . $route, '\\'.$controller . '@update');
		self::delete('/' . $route, '\\'.$controller . '@destroy');
	}
	...
	public function middleware($string) {
		...
		if(in_array($string, array_keys($middleWare->routerMiddleWare))){
			if(strstr($request->getPath(), $self->segments[0])){
				$routeMiddleWare = (new Resolve)->resolve($middleWare->routerMiddleWare[$string]);//new $middleWare->routerMiddleWare[$string](Session::instance());
				$self = self::instance();
